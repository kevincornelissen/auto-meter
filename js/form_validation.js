function validateForm() {
    ...
    $("#autoInputSaab").parent().parent().removeClass("has-error");
    if ($("input[name='auto']:checked").length != 1 || ($("input[name='auto']:checked").val() != "Saab" && $("input[name='auto']:checked").val() != "Sharan")){
        $("#autoInputSaab").parent().parent().addClass("has-error");
        if (status!== false){ status = false;}
    }
